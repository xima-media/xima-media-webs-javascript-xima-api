...
this.initGoogleMaps=function(k){if(e==true){return;}if(!h){console.log(g.msg.MissingMapCanvas);return;}e=true;if(i.mapOptions){this.addMapOptions(i.mapOptions);
}c=new google.maps.Map(h,b);this.applyMapData(k);for(var l in a){if(a.hasOwnProperty(l)){a[l].setMap(c);}}};this.applyMapData=function(l){if(!c){console.log(g.msg.MissingMap);
return this;}if(jQuery.isEmptyObject(i)||!i.points){console.log(g.msg.MissingMapDataProperty);return this;}var l=(typeof l!=="boolean")?true:l;if(l){var k=new google.maps.LatLngBounds();
}var r=(!jQuery.isEmptyObject(i)&&i.points)?Object.keys(i.points).length:0;var q=[];var s;var o;var t;var n;for(var p=0;p<r;p++){if(!i.points[p].lat||!i.points[p].lng){continue;
}o=new google.maps.LatLng(parseFloat(i.points[p].lat),parseFloat(i.points[p].lng));s=(i.points[p].title)?i.points[p].title:"";t=(i.points[p].icon)?i.points[p].icon:null;
this.addMarker(o,s,t);n=d.length-1;if(l){k.extend(d[n].position);}d[n].setMap(c);}if(l&&d.length>0){c.fitBounds(k);var m=google.maps.event.addListener(c,"zoom_changed",function(){if(c.getZoom()>b.maxZoom){c.setZoom(b.maxZoom);
...
}return this;};this.setMapCanvas=function(k){h=jQuery(k).get(0);return this;};this.addLayers=function(k){if(typeof k==="object"){for(var l in k){a[l]=new google.maps.KmlLayer({url:k[l],preserveViewport:true});
